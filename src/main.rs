...
fn init_tracing() -> TracerProvider {
    ...
    let service_name = env::var("OTEL_SERVICE_NAME").unwrap_or_else(|_| "roxy".to_string());
...
fn handle_get(stream: &mut TcpStream, req: &httparse::Request) -> io::Result<()> {
    ...
    let (hostname, port) = if let Some(colon) = host.rfind(':') {
        (
            &host[..colon],
            host[colon + 1..].parse::<i64>().unwrap_or(80),
        )
    } else {
        (host, 80i64)
    };
    ...
    match TcpStream::connect(&target) {
        Ok(mut remote) => {
            ...
            if path.starts_with("http://") {
                path = path[7..]
                    .find('/')
                    .map_or("/".into(), |pos| path[7 + pos..].to_string());
            }
...
fn handle_connect(stream: &mut TcpStream, req: &httparse::Request) -> io::Result<()> {
    ...
    let (hostname, port) = if let Some(colon) = target.rfind(':') {
        (
            &target[..colon],
            target[colon + 1..].parse::<i64>().unwrap_or(443),
        )
    } else {
...
mod tests {
    ...
    fn test_http() {
        ...
        assert!(
            combined.contains("Connected to 127.0.0.1"),
            "did not see expected connection line: {}",
            combined
        );
        assert!(
            combined.contains("HTTP/1.1 301") || combined.contains("HTTP/1.0 301"),
            "did not see 301 response: {}",
            combined
        );
        assert!(
            combined.contains("Location: https://github.com/jamesmoriarty"),
            "did not see Location header: {}",
            combined
        );
        assert!(
            combined.to_lowercase().contains("connection: close"),
            "did not see connection: close: {}",
            combined
        );
        handle.join().unwrap();
    }
    ...
    fn test_https() {
        ...
        assert!(
            combined.contains("> CONNECT github.com:443 HTTP/1.1")
                || combined.contains("CONNECT github.com:443 HTTP/1.1"),
            "did not see CONNECT: {}",
            combined
        );
        assert!(
            combined.contains("HTTP/1.1 200 Connection Established")
                || combined.contains("HTTP/1.1 200"),
            "did not see 200: {}",
            combined
        );
        assert!(
            combined.contains("HTTP/2 200") || combined.contains("HTTP/1.1 200"),
            "did not see inner 200: {}",
            combined
        );
        assert!(
            combined.to_lowercase().contains("ssl connection")
                || combined.to_lowercase().contains("tls handshake")
                || combined.to_lowercase().contains("alpn"),
            "did not see TLS info: {}",
            combined
        );
        handle.join().unwrap();
    }
